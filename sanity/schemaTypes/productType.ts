...
export const productType = defineType({
  ...
  fieldsets: [
    ...
    { name: "details", title: "🧩 Features & Specs", options: { collapsible: true, collapsed: false } },
  ...
  fields: [
    ...
    defineField({
      ...
      fieldset: "pricing",
      description:
        "Optional selectable prices (e.g., Standard Loss, Low Loss). If set, shoppers pick one on the product page.",
      of: [
        defineArrayMember({
          name: "priceOption",
          type: "object",
          fields: [
            ...
            defineField({
              name: "dealerPrice",
              title: "Dealer Price (optional)",
              type: "number",
              description:
                "Leave blank to auto-derive from User Price using current markup settings.",
              validation: (Rule) => Rule.min(0),
            }),
            defineField({
              name: "isDefault",
          ...
          preview: {
            ...
            prepare({ title, price, isDefault }) {
              ...
              return {
                title: title || "Price option",
                subtitle: `${isDefault ? "Default • " : ""}${priceLabel}`,
              };
            },
          },
        }),
      ...
      validation: (Rule) =>
        Rule.custom((options) => {
          ...
          return defaultCount > 1 ? "Only one price option can be marked default." : true;
        }),
    }),
    defineField({
      name: "discount",
    ...
    defineField({
      ...
      type: "array",
      fieldset: "details",
      description: "Short bullets that summarize the product's standout features.",
      of: [
        defineArrayMember({
    ...
    defineField({
      ...
      type: "array",
      fieldset: "details",
      description: "Structured key/value specifications for technical buyers.",
      of: [
        defineArrayMember({
          name: "specification",
          type: "object",
