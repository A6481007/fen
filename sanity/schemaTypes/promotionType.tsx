...
import { PatchEvent, defineArrayMember, defineField, defineType, set } from "sanity";
...
type PromotionTemplateKey = keyof typeof PROMOTION_TEMPLATES;
...
const buildCampaignId = (name?: string | null) => {
  const base = slugifyId(name || "promo");
  const timestamp = new Date().toISOString().replace(/[-:T.Z]/g, "").slice(0, 12);
  const random = Math.random().toString(36).slice(-4);
  return [base || "promo", timestamp, random].filter(Boolean).join("-");
};
...
const applyPromotionTemplateDefaults = (
  doc: Record<string, any>,
  templateKey: PromotionTemplateKey
): PatchEvent | null => {
  const template = PROMOTION_TEMPLATES[templateKey];
  if (!template) return null;

  const patches: ReturnType<typeof set>[] = [];
  const ensureObject = (path: (string | number)[]) => {
    const current = readPath(doc, path);
    if (!current || typeof current !== "object") {
      patches.push(set({}, path));
    }
  };
  ...
  maybeSet(["discountType"], defaults.discountType);
  if (defaults.discountValue !== undefined) maybeSet(["discountValue"], defaults.discountValue);
  if (defaults.minimumOrderValue !== undefined) maybeSet(["minimumOrderValue"], defaults.minimumOrderValue);
  if (defaults.maximumDiscount !== undefined) maybeSet(["maximumDiscount"], defaults.maximumDiscount);
  if (defaults.buyQuantity !== undefined) maybeSet(["buyQuantity"], defaults.buyQuantity);
  if (defaults.getQuantity !== undefined) maybeSet(["getQuantity"], defaults.getQuantity);
  if (defaults.badgeLabel !== undefined) maybeSet(["badgeLabel"], defaults.badgeLabel);
  if (defaults.badgeColor !== undefined) maybeSet(["badgeColor"], defaults.badgeColor);
  if (defaults.ctaText !== undefined) maybeSet(["ctaText"], defaults.ctaText);

  ensureObject(["urgencyTrigger"]);
  if (defaults.showCountdown !== undefined) {
    maybeSet(["urgencyTrigger", "showCountdown"], defaults.showCountdown);
  }
  if (defaults.urgencyMessage !== undefined) {
    maybeSet(["urgencyTrigger", "urgencyMessage"], defaults.urgencyMessage);
  }

  ensureObject(["targetAudience"]);
  if (defaults.segmentType !== undefined) {
    maybeSet(["targetAudience", "segmentType"], defaults.segmentType);
  }
  if (defaults.inactivityDays !== undefined) {
    maybeSet(["targetAudience", "inactivityDays"], defaults.inactivityDays);
  }
  if (defaults.minLTVThreshold !== undefined) {
    maybeSet(["targetAudience", "minLTVThreshold"], defaults.minLTVThreshold);
  }

  return patches.length ? PatchEvent.from(patches) : null;
};

const TemplateGuide = (props: any) => {
  const doc = (props.document || {}) as Record<string, any>;
  const type = doc?.type as PromotionTemplateKey | undefined;
  const template = type ? PROMOTION_TEMPLATES[type] : undefined;
  const appliedRef = useRef<PromotionTemplateKey | null>(null);

  useEffect(() => {
    if (!type || appliedRef.current === type) return;
    const patch = applyPromotionTemplateDefaults(doc, type);
    if (patch) {
      props.onChange?.(patch);
    }
    appliedRef.current = type;
  }, [doc, props.onChange, type]);

  if (!template) {
    return props.renderDefault?.(props) ?? null;
  }
  ...
  return (
    <div style={{ display: "grid", gap: "8px" }}>
      ...
      >
        ...
        <p style={{ margin: "6px 0" }}>{template.description}</p>
        ...
        >
          Apply template defaults to empty fields
        </button>
      </div>
    </div>
  );
...
export const promotionType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: "Choose a template to auto-configure your promotion. Defaults fill empty fields; you can customize after selection.",
      options: {
        list: PROMOTION_TYPE_OPTIONS,
        layout: "radio",
      },
      components: { input: TemplateGuide },
    ...
    defineField({
      ...
      initialValue: (params, context) =>
        buildCampaignId((context?.document as { name?: string })?.name),
      validation: (Rule) =>
        Rule.required()
          .regex(/^[a-z0-9-]+$/, {
            name: "slug-safe",
          ...
          .custom(async (value, context) => {
            if (!value) return "Campaign ID is required";
            const client = context.getClient?.({ apiVersion: "2023-10-01" });
            if (!client) return true;
            const currentId = (context.document as { _id?: string })?._id;
            const count = await client.fetch<number>(
              'count(*[_type == "promotion" && campaignId == $campaignId && _id != $currentId])',
              { campaignId: value, currentId: currentId ?? "" }
            );
            return count === 0 || "Campaign ID must be unique";
          }),
    }),
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.min(0).custom((value, context) => {
          const doc = getDoc(context);
          const dt = getDiscountType(doc);
          if (dt === "freeShipping" && (!value || value <= 0)) {
            return "Free shipping requires a minimum order value above 0";
          }
          return true;
        }),
    }),
    ...
    defineField({
      ...
      validation: (Rule) => Rule.custom((value, context) => {
        ...
        if (!Number.isInteger(value)) return "Buy quantity must be a whole number";
        return true;
      }),
    }),
    ...
    defineField({
      ...
      validation: (Rule) => Rule.custom((value, context) => {
        ...
        if (!Number.isInteger(value)) return "Get quantity must be a whole number";
        return true;
      }),
    }),
      ...
      of: [
        defineArrayMember({
          ...
          fields: [
            ...
            defineField({
              name: "variantId",
              title: "Variant ID (optional)",
              type: "string",
              description: "Specify a variant to auto-add when this product has variants.",
            }),
            defineField({
              name: "isFree",
    ...
    defineField({
      ...
      title: "Default Products (auto-add)",
      type: "array",
      group: "discount",
      fieldset: "discountConfig",
      description: "Set items to auto-add for non-bundle promotions.",
      hidden: ({ document }) => isBxgy(document as any),
      of: [
        defineArrayMember({
          type: "object",
          fields: [
            ...
            defineField({
              ...
              title: "Variant ID (optional)",
              type: "string",
              description: "If the product requires a variant, set it here for one-click apply.",
            }),
          ],
          preview: {
            select: {
      ...
      validation: (Rule) =>
        Rule.custom((value, context) => {
          ...
          const hasProduct = value.every((item) => Boolean((item as { product?: any })?.product));
          if (!hasProduct) return "Each auto-add entry needs a product.";
          return true;
        }),
    }),
    ...
    defineField({
      ...
      fields: [
        ...
        defineField({
          ...
          title: "Maximum Lifetime Value",
          type: "number",
          description: "Optional ceiling for eligibility (0 = none)",
          hidden: ({ parent }) => parent?.segmentType !== "vip",
          initialValue: 0,
          validation: (Rule) => Rule.min(0),
        }),
        defineField({
          ...
          title: "Cart Abandonment Window (days)",
          type: "number",
          description: "For cart-abandoner segments: days since an abandoned cart",
          hidden: ({ parent }) => parent?.segmentType !== "cartAbandoner",
          initialValue: 7,
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.required().custom((value) => validateCtaLabel(value as string | undefined, { isPrimary: true })),
    }),
    ...
    defineField({
      ...
      group: "schedule",
      fieldset: "timing",
      description: "Set the timezone you authored dates in to avoid misaligned launches.",
      options: {
        list: [
          { title: "UTC", value: "UTC" },
    ...
    ...buildBannerFields({
      initialPlacement: "promotionspagehero",
      group: "schedule",
      fieldset: "timing",
    }),
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((value) => {
          ...
          return validateCtaLabel(value as string | undefined, { isPrimary: true });
        }),
    }),
    defineField({
      name: "variantCtaB",
    ...
    defineField({
      name: "variantDesign",
      title: "Variant Design Token",
      type: "string",
      group: "advanced",
      fieldset: "variants",
      description: "Optional design token or theme identifier for experiments.",
      hidden: ({ document }) => document?.variantMode === "control",
    }),
    ...
    defineField({
      ...
      group: "advanced",
      readOnly: true,
      description: "Filled by automation or API. Leave blank in Studio.",
    }),
    defineField({
      name: "lastModifiedBy",
  ...
  preview: {
    ...
    prepare({
      ...
      const bundleCount = Array.isArray(defaultBundleItems) ? defaultBundleItems.length : 0;
      const defaultsCount = Array.isArray(defaultProducts) ? defaultProducts.length : 0;
      const autoLabel =
        bundleCount > 0
          ? `Bundle: ${bundleCount} items`
          : defaultsCount > 0
            ? `Auto-add: ${defaultsCount} items`
            : "";
      const windowLabel =
        startDate && endDate
      ...
      const subtitleParts = [template?.label || type, discountLabel].filter(Boolean);
      if (autoLabel) subtitleParts.push(autoLabel);
      if (windowLabel) subtitleParts.push(windowLabel);
