...
import { PreviewDocumentAction } from "./sanity/actions/previewAction";
import LocaleLanguageFilter from "./sanity/studio/src/components/LocaleLanguageFilter";
...
const previewableSchemaTypes = new Set(["insight"]);
...
const resolveDocumentActions = (prev: any, context: any) => {
  ...
  if (previewableSchemaTypes.has(schemaType)) {
    return [PreviewDocumentAction, ...prev];
  }
...
export default defineConfig({
  ...
  document: {
    ...
    unstable_languageFilter: (prev, context) => {
      if (["insight", "news", "event"].includes(context.schemaType)) {
        return [...prev, LocaleLanguageFilter];
      }
      return prev;
    },
