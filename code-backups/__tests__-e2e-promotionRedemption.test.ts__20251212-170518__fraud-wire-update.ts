...
vi.mock("@/lib/promotions/anomalyDetection", () => {
  const DEFAULT_CONFIG = {
    velocity: { campaignPerMinuteWarning: 5, campaignPerMinuteCritical: 12, userPerHour: 6, userPerDay: 14 },
    geographic: { maxCountriesPerHour: 2, flaggedCountries: [] },
    cartGaming: { nearMinimumBuffer: 5, windowMinutes: 30, minEvents: 3 },
    accountAge: { newAccountHours: 24, highValuePromoThreshold: 50 },
    deviceFingerprint: { uniqueUsersThreshold: 3, windowHours: 24 },
    actions: { pauseOnCritical: true },
  };

  const evaluateAllChecks = vi.fn(async () => {
    ...
    const severity = (next.severity ?? "medium") as "low" | "medium" | "high" | "critical";
    const type = (next.anomalyType as any) ?? "velocity_campaign";
    const action = (next.action as any) ?? "flag_for_review";
    const finding = {
      type,
      severity,
      reason: next.details ?? "anomaly detected",
      evidence: {},
      action,
      shouldBlock: Boolean(next.shouldBlock),
      shouldPause: action === "pause_promotion",
    };
  ...
  return {
    DEFAULT_CONFIG,
    evaluateAllChecks,
    autoPausePromotion: vi.fn(async () => {}),
  };
});
...
vi.mock("@/lib/rateLimit", () => {
  ...
  const RATE_LIMIT_CONFIGS = {
    redemption: { limit: 10, windowMs: 60_000 },
    track: { limit: 10, windowMs: 60_000 },
  };
  const getClientIP = (request: Request) => request.headers.get("x-real-ip") ?? "127.0.0.1";
  const withRateLimit = (handler: any, configName: keyof typeof RATE_LIMIT_CONFIGS) => {
    return async (req: Request) => {
      const config = RATE_LIMIT_CONFIGS[configName] ?? { limit: 100, windowMs: 60_000 };
      const now = Date.now();
      const key = `${configName}:${getClientIP(req)}`;
      const existing =
        rateLimitState.counts.get(key) ??
        { count: 0, resetAt: now + config.windowMs, limit: config.limit ?? 100 };
      ...
      existing.count += 1;
      existing.limit = rateLimitState.overrideLimit ?? existing.limit;
      rateLimitState.counts.set(key, existing);

      const effectiveLimit = existing.limit ?? config.limit ?? 100;
  ...
  return { checkRateLimit, RATE_LIMIT_CONFIGS, getClientIP, withRateLimit };
});
...
describe("E2E: Promotion Redemption Flow", () => {
  ...
  describe("Fraud Prevention", () => {
    ...
    it("detects velocity anomaly", async () => {
      ...
      const body = await response.json();
      expect(body.fraud?.decision).toBe("deny");
    });
