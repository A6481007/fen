...
import { getActivePromotions, getDealsAndLegacyHotProducts } from "@/sanity/queries";
import type { Product, PROMOTIONS_LIST_QUERYResult } from "@/sanity.types";
...
type DealWithSource = Deal & { source: "deal" };
type LegacyHotDeal = { source: "legacy"; product?: Product | null };

const isDealSource = (entry: unknown): entry is DealWithSource =>
  Boolean(entry && typeof entry === "object" && (entry as { source?: string }).source === "deal");

const isLegacyHotDeal = (entry: unknown): entry is LegacyHotDeal =>
  Boolean(entry && typeof entry === "object" && (entry as { source?: string }).source === "legacy");
...
export const metadata: Metadata = {
  ...
  description:
    "Explore featured deals, flash sales, and seasonal promotions. Legacy hot deals remain available during the transition.",
  openGraph: {
    title: "Featured Deals & Promotions | ShopCart",
    description:
      "Discover our featured deals, live flash sales, and limited-time hot offers while we transition from legacy deals.",
  },
  alternates: {
    canonical: "/deal",
  },
...
const DealPage = async () => {
  ...
  const dealsWithLegacy = Array.isArray(dealsData) ? dealsData : [];
  const deals = dealsWithLegacy.filter(isDealSource);
  const legacyHotProducts = dealsWithLegacy
    .filter(isLegacyHotDeal)
    .map((entry) => entry.product)
    .filter(Boolean) as Product[];
  const hasPromotions = sortedPromotions.length > 0;
  const hasDeals = deals.length > 0;
  const hasLegacyHotProducts = legacyHotProducts.length > 0;
  const showLegacyHotDeals = hasLegacyHotProducts || (!hasPromotions && !hasDeals);
  const showEmptyState = !hasPromotions && !hasDeals && !hasLegacyHotProducts;
      ...
      <Container className="space-y-8 py-10">
        <header className="text-center space-y-2">
          ...
          <p className="text-gray-600">
            Flash sales and seasonal campaigns appear first. Featured deals use the new deal system;
            legacy hot deals remain available during the transition but will be phased out soon.
          </p>
        </header>
        ...
        {showLegacyHotDeals ? (
          <section className="space-y-4">
            ...
            <p className="text-sm text-amber-700">
              Legacy hot deals are being phased out as we transition to the new Featured deals system.
            </p>

            {hasLegacyHotProducts ? (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                {legacyHotProducts.map((product) => (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg font-semibold text-gray-900">
                      No legacy hot deals right now
                    </h3>
                    <p className="text-sm text-gray-600">
                      New featured deals will continue to roll out. Legacy offers will disappear over time.
                    </p>
                  </div>
                  <Button asChild>
