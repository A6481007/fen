...
import {
  generateCreativeWorkSeasonSchema,
  generateEventSchema,
...
type NewsDocumentInput = Parameters<typeof generateNewsArticleSchema>[0];
type NewsItem = NewsDocumentInput & {
  blogcategories?: { title?: string | null }[];
  isEvent?: boolean;
...
const NewsPage = async ({
  ...
  const newsItems: NewsItem[] = (await getAllNews(12)) ?? [];
  ...
  const getNewsExcerpt = (item: NewsItem) =>
    item?.summary?.trim() || extractDescription(item?.body || []);

  const getReadingTime = (item: NewsItem) =>
    calculateReadingTime(getNewsExcerpt(item));

  const articleSchemas = newsItems
    .map((item) => [
      generateNewsArticleSchema(item),
      item?.contentType === "event" || item?.isEvent
        ? generateEventSchema(item)
        : null,
    ])
    .flat()
    .filter(Boolean);

  const itemListSchema = generateItemListSchema(newsItems, "News Hub", {
    basePath: "/news",
  });

  const seasonSchema = generateCreativeWorkSeasonSchema({
    title: "ShopCart Newsroom Releases",
    summary:
      "Curated announcements, product updates, and media highlights from the ShopCart team.",
    slug: { current: "" },
    episodes: newsItems,
  });
