...
import { DashboardSkeleton } from "@/components/admin/promotions/DashboardSkeleton";
import {
  ChannelPerformance,
  DerivedMetrics,
  DevicePerformance,
...
export default async function PromotionAnalyticsPage({ params, searchParams }: PageProps) {
  ...
  const [promotion, analyticsRaw, variantAnalyticsRaw] = await Promise.all([
  ...
  const promotionSummary: PromotionSummary = {
    name: promotion.name || campaignId,
    status: promotion.status || "active",
    variantMode: promotion.variantMode,
    startDate: promotion.startDate,
    endDate: promotion.endDate,
    budgetCap: toNumber(promotion.budgetCap, 0),
    usageLimit: toNumber(promotion.usageLimit, 0),
    timeRemaining: toNumber(promotion.timeRemaining, 0),
    minimumOrderValue: toNumber(promotion.minimumOrderValue, 0),
  };
  ...
  const metrics = buildDerivedMetrics(analytics, promotionSummary, campaignId);
  const funnel = buildFunnel(metrics);
  const dailySeries = buildDailySeries(metrics, promotionSummary, campaignId);
  ...
  const timeRemainingMs =
    typeof promotionSummary.timeRemaining === "number"
      ? Math.max(0, promotionSummary.timeRemaining)
      : promotionSummary.endDate
        ? Math.max(0, new Date(promotionSummary.endDate).getTime() - Date.now())
        : null;
  ...
  const snapshot: AnalyticsSnapshot = {
    ...
    budgetCap: promotionSummary.budgetCap ?? 0,
    usageLimit: promotionSummary.usageLimit ?? 0,
    timeRemainingMs,
  };

  const variantAnalytics = normalizeVariantAnalytics(variantAnalyticsRaw);
