...
describe("PromotionEngine", () => {
  ...
  describe("matchesSegment", () => {
    ...
    it("returning customer (3 orders) matches 'returning' segment", () => {
      ...
      const user = mockUser({ ordersCount: 3, lastPurchaseAt: new Date(Date.now() - 86400000 * 10).toISOString() });
      const matchesSegment = (engine as any).matchesSegment.bind(engine);
    ...
    it("VIP customer (10 orders, $600 LTV) matches 'vip' segment", () => {
      ...
      const user = mockUser({
        ordersCount: 10,
        ltv: 600,
        lastPurchaseAt: new Date(Date.now() - 86400000 * 5).toISOString(),
      });
      const matchesSegment = (engine as any).matchesSegment.bind(engine);
    ...
    it("User with abandoned cart matches 'cartAbandoner' when threshold met", () => {
      ...
      const user = mockUser({
        ordersCount: 2,
        lastAbandonedCartAt: new Date(Date.now() - 86400000 * 2).toISOString(),
        lastAbandonedCartValue: 180,
      });
      const context = mockContext({
        sessionData: { lastAbandonedCartAt: new Date(Date.now() - 86400000 * 2).toISOString() },
      });
      const matchesSegment = (engine as any).matchesSegment.bind(engine);
    ...
    it("Inactive user (60 days since purchase) matches 'inactive' segment", () => {
      ...
      const user = mockUser({
        ordersCount: 2,
        lastPurchaseAt: new Date(Date.now() - 86400000 * 60).toISOString(),
      });
      const matchesSegment = (engine as any).matchesSegment.bind(engine);
    ...
    it("User does NOT match segment they don't qualify for", () => {
      ...
      const user = mockUser({ ordersCount: 1, ltv: 100 });
      const matchesSegment = (engine as any).matchesSegment.bind(engine);
  ...
  describe("rankPromotions", () => {
    ...
    const buildEligible = (
      ...
      promotion: mockPromotion(promoOverrides),
    ...
    it("orders higher priority promotions first", () => {
      ...
      expect(ranked[0].promotion.campaignId).toBe("high");
    });
    ...
    it("orders by higher discount when priorities match", () => {
      ...
      expect(ranked[0].promotion.campaignId).toBe("bigger");
    });
    ...
    it("orders by end date when priority and discount tie", () => {
      ...
      expect(ranked[0].promotion.campaignId).toBe("soon");
    });
    ...
    it("boosts relevance for promos scoped to the active product page", () => {
      ...
      expect(ranked[0].promotion.campaignId).toBe("product-specific");
    });
  });
  ...
  describe("assignVariant", () => {
    it("returns control variant when variantMode is 'control'", () => {
      ...
      const variant = engine.assignVariant(promo as any, "user-123");
    ...
    it("uses split percent to deterministically bucket users", () => {
      ...
      const bucket = (engine as any).hashUserToBucket("user-split", promo.campaignId);
      const variant = engine.assignVariant(promo as any, "user-split");
    ...
    it("keeps variant consistent for the same user and campaign", () => {
      ...
      const first = engine.assignVariant(promo as any, "user-repeat");
      const second = engine.assignVariant(promo as any, "user-repeat");
    ...
    it("assigns different variants for different users over time", () => {
      const promo = mockPromotion({ variantMode: "split", splitPercent: 50 });
      const baseVariant = engine.assignVariant(promo as any, "user-0").variant;

      let different: string | null = null;
      for (let i = 1; i < 50; i += 1) {
        const next = engine.assignVariant(promo as any, `user-${i}`).variant;
        if (next !== baseVariant) {
          different = next;
          break;
        }
