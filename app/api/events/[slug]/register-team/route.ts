...
const EVENT_BY_SLUG_FOR_TEAM_REGISTRATION = `
  *[_type == "event" && slug.current == $slug && (!defined(publishStatus) || publishStatus == "published")][0]{
    _id,
    title,
    "slug": slug.current,
    date,
...
export async function POST(request: Request, { params }: { params: { slug: string } }) {
  ...
  const slug = normalizeString(params?.slug) || normalizeString(body?.slug);
  if (!slug) {
  ...
  try {
    ...
    if (userIdentifier) {
      const accessEvent = {
        ...
        statusOverride: (event.statusOverride as EventStatus) ?? undefined,
      };
      const alreadyRegistered = await isUserEventAttendee(userIdentifier, accessEvent, {
    ...
    revalidate("events");
    revalidate(`event:${eventSlugValue}`);
