...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId: sanityUserId, customerCode } =
      await ensureCustomerCodeForUser({
        clerkUserId: userId,
        email: userEmail,
    ...
    const addressPayload: Address = {
      ...
      lineId: normalizeOptionalString(body.lineId),
    ...
    const newAddress = await backendClient.create({
      ...
      lineId: addressPayload.lineId ?? null,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { customerCode } = await ensureCustomerCodeForUser({
      clerkUserId: userId,
      email: userEmail,
      firstName: user.firstName || "",
      lastName: user.lastName || "",
