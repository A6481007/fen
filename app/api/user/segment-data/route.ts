...
import { adminDb, Timestamp, firebaseAdminAvailable } from "@/lib/firebaseAdmin";
import {
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      sessionUserId = authResult?.userId ?? null;
    } catch (error) {
      console.warn("[segment-data] auth unavailable", error);
      sessionUserId = null;
    }
    ...
    if (!firebaseAdminAvailable || !adminDb) {
      const segmentData = buildDefaultSegmentData();
      ...
      return NextResponse.json({
        ...
        promoUsage: [],
        disabled: true,
        message: "User segmentation fallback used (admin SDK not configured)",
      });
    }
    ...
    if (userDoc.exists) {
      ...
      segmentData = {
        ...
        lastCartValue: toNumber(data?.lastCartValue),
