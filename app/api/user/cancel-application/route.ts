...
export async function POST(request: NextRequest) {
  try {
    ...
    if (applicationType === "premium") {
      ...
      // Cancel dealer application - only if pending or rejected
      if (
        existingUser.businessStatus !== "pending" &&
        existingUser.businessStatus !== "rejected"
      ) {
        return NextResponse.json(
          { error: "Cannot cancel dealer application in current status" },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json({
        success: true,
        message: "Dealer application cancelled. You can apply again.",
        user: result,
      });
    } else {
      return NextResponse.json(
