...
export class PromotionEngine {
  ...
  checkEligibility(
    ...
    const scopeMatch = this.matchesProductScope(context, promo);
    if (scopeMatch) {
  ...
  private matchesProductScope(
    ...
    (targetAudience.products ?? []).forEach((product) => {
      if (product?._id) allowedProducts.add(product._id);
    });
    (promo.products ?? []).forEach((product) => {
      if (product?._id) allowedProducts.add(product._id);
    });
    (promo.defaultProducts ?? []).forEach((item) => {
      if (item?.product?._id) allowedProducts.add(item.product._id);
    });
    (promo.defaultBundleItems ?? []).forEach((item) => {
      if (item?.product?._id) allowedProducts.add(item.product._id);
    });

    (targetAudience.excludedProducts ?? []).forEach((product) => {
      if (product?._id) excludedProducts.add(product._id);
    });
    (promo.excludedProducts ?? []).forEach((product) => {
      if (product?._id) excludedProducts.add(product._id);
    });

    (targetAudience.categories ?? []).forEach((category) => {
      if (category?._id) allowedCategories.add(category._id);
    });
