      ...
      <Card className="border border-gray-100 shadow-md">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg text-shop_dark_green">
            ...
            {t("client.news.attachments.title")}
          </CardTitle>
          <p className="text-sm text-gray-600">
            {t("client.news.attachments.empty")}
          </p>
        </CardHeader>
      </Card>
    );
  ...
  return (
    <Card className="border border-gray-100 shadow-md">
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          {t("client.news.attachments.subtitle")}
        </p>
      </CardHeader>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                <AccessControl
                  accessible={!isLocked}
                  accessLevel={attachment.status === "event_locked" ? "event-locked" : "public"}
                  fallback={
                    <>
                      ...
                      >
                        ...
                        {t("client.news.attachments.locked")}
                      </Button>
                      {attachment.status === "event_locked" && eventHref ? (
                        <Button
                          asChild
                          variant="outline"
                        ...
                        >
                          <Link
                            ...
                            aria-label={
                              linkedEventTitle
                                ? t("client.news.attachments.registerEventAria", {
                                    title: linkedEventTitle,
                                  })
                                : t("client.news.attachments.registerEventAriaFallback")
                            }
                          >
                            {t("client.news.attachments.registerEvent")}
                          </Link>
                        </Button>
                      ) : null}
                    </>
                    ...
                    {downloadUrl ? (
                      ...
                      >
                        <Link href={downloadUrl} target="_blank" rel="noopener noreferrer" download>
                          <Download className="mr-2 h-4 w-4" aria-hidden="true" />
                          {t("client.news.attachments.download")}
                        </Link>
                      </Button>
                    ) : (
                      <Button
                      ...
                      >
                        <Download className="mr-2 h-4 w-4" aria-hidden="true" />
                        {t("client.news.attachments.unavailable")}
                      </Button>
                    )}

                    {!downloadUrl ? (
                      <p className="text-xs text-gray-500 sm:text-right">
                        {t("client.news.attachments.pending")}
                      </p>
                    ) : null}
                  </>
                </AccessControl>
