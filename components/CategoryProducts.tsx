...
const CategoryProducts = ({ categories, slug, initialProducts }: Props) => {
          ...
          >
            {hasChildren ? (
              <button
                ...
                aria-label={
                  expanded
                    ? t("client.products.slug.categoryTree.collapse", {
                        defaultValue: "Collapse category",
                      })
                    : t("client.products.slug.categoryTree.expand", {
                        defaultValue: "Expand category",
                      })
                }
            ...
            >
              {node.title ||
                node.slug?.current ||
                t("client.products.slug.fallback.category", { defaultValue: "Category" })}
            </button>
  ...
  const activePathLabel =
    activePath.length > 0
      ...
      : t("client.products.slug.fallback.thisCategory", {
      ...
      <div className="w-full lg:w-80 flex-shrink-0">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 bg-gray-50 border-b border-gray-200 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-brand-black-strong">
                {t("client.products.slug.categoryTree.title", {
                  defaultValue: "Category layers",
                })}
              </p>
              <p className="text-xs text-gray-600">
                {t("client.products.slug.categoryTree.subtitle", {
                  defaultValue: "Navigate parents, children, and leaf nodes.",
                })}
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <h3 className="text-lg font-semibold text-brand-black-strong">
                    {t("client.products.slug.results.title", {
                      defaultValue: "Products in {{label}}",
                      label: activePathLabel,
                    })}
                  </h3>
                  <p className="text-sm text-gray-600">
                    {t("client.products.slug.results.count", {
                      defaultValue: "{{count}} product found",
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  ...
                  <span>{t("client.products.slug.results.grid", { defaultValue: "Grid View" })}</span>
                </div>
              </div>
            </div>
