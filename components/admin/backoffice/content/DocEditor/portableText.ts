...
const getAssetRef = (value: unknown): string | undefined => {
...
export function tiptapToPortableText(json?: JSONContent | null): PortableTextBlock[] {
  ...
  const walk = (node: JSONContent, listContext?: { type: "bullet" | "number"; level: number }) => {
    ...
    switch (node.type) {
      ...
      default: {
        const rawAttr = (node.attrs as { raw?: unknown } | undefined)?.raw;
        const raw = coerceObject(rawAttr);
        if (raw) {
          blocks.push(raw as PortableTextBlock);
        } else {
          blocks.push({
...
function portableBlockToNode(
  ...
  if (blockType === "blockImage") {
    ...
    const assetId = getAssetRef(img.image);
    ...
    return {
      ...
      attrs: {
        ...
        title: img.caption ?? "",
        credit: (img as { credit?: string }).credit ?? "",
  ...
  if (blockType === "image") {
    const assetId = getAssetRef((block as { asset?: { _ref?: string } }).asset);
    ...
    return {
      ...
      attrs: {
        ...
        title: (block as { caption?: string }).caption ?? "",
        credit: (block as { credit?: string }).credit ?? "",
        originalType: "image",
        blockKey: block._key ?? null,
      },
    };
  ...
  if (blockType !== "block") {
    return {
      ...
      attrs: {
        ...
        blockData: block,
        label: (block as { title?: string })?.title || blockType || "Block",
      },
    };
  }
...
function buildSpanNodes(children: PortableTextSpan[], markDefs: PortableTextMarkDef[]): JSONContent[] {
  ...
  children.forEach((span) => {
    ...
    const textString = textValue === undefined || textValue === null ? "" : String(textValue);
    ...
    nodes.push({ type: "text", text: textString, marks: marks!.length ? marks : undefined });
  });
