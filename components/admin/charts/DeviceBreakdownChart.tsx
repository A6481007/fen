  ...
  return (
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
          ...
          <PieChart>
            ...
            >
              {data.map((entry, index) => (
                <Cell
                  key={entry.device}
                  fill={`var(--color-${entry.device}, ${palette[index % palette.length]})`}
                  stroke="#fff"
                  strokeWidth={1}
                />
              ))}
              ...
              content={
                <ChartTooltipContent
                  formatter={(value, name, item) => {
                    ...
                    const label =
                      chartConfig?.[item.payload?.device as keyof ChartConfig]?.label || name;

                    return [`${numericValue.toLocaleString()} (${percent.toFixed(1)}%)`, label];
                  }}
                  labelFormatter={(value) => `Device: ${value as string}`}
                />
              }
            />
            <ChartLegend
        ...
        <div className="space-y-2" aria-label="Device distribution details" role="list">
          {data.map((item, index) => {
            const percentage = total > 0 ? (item.value / total) * 100 : 0;
            const color = palette[index % palette.length];

            return (
              <div key={item.device} className="space-y-1" role="listitem">
